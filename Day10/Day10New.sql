...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 19, 2018 at 05:52 PM
...
--
CREATE TABLE IF NOT EXISTS `d10_v1` (
`Code` int(3)
,`Name` varchar(255)
,`Price` decimal(10,0)
,`Manufacturer` int(3)
...
--
CREATE TABLE IF NOT EXISTS `d10_v8` (
`code` int(3)
,`product` varchar(255)
,`manufecturer` varchar(50)
);
...
INSERT INTO `j3` (`ID`, `LastName`, `FirstName`, `Age`, `CityID`) VALUES
(1, 'Patel', 'Harsh', 22, 1);
...
('hh123', NULL, NULL, NULL, NULL, NULL);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `d10_v8` AS select `products`.`Code` AS `code`,`products`.`Name` AS `product`,`manu`.`Name` AS `manufecturer` from (`manu` join `products` on((`products`.`Code` = `manu`.`Code`)));
...
ALTER TABLE `countries`
  ADD PRIMARY KEY (`COUNTRY_ID`), ADD KEY `COUNTR_REG_FK` (`REGION_ID`);

--
-- Indexes for table `departments`
--
ALTER TABLE `departments`
  ADD PRIMARY KEY (`DEPARTMENT_ID`), ADD KEY `DEPT_MGR_FK` (`MANAGER_ID`), ADD KEY `DEPT_LOCATION_IX` (`LOCATION_ID`);
...
--
ALTER TABLE `j3`
  ADD PRIMARY KEY (`ID`), ADD KEY `fk_persons` (`CityID`);
...
ALTER TABLE `products`
  ADD PRIMARY KEY (`Code`), ADD KEY `FK_PRODUCTS` (`Manufacturer`);

--
-- Indexes for table `regions`
--
ALTER TABLE `regions`
  ADD PRIMARY KEY (`REGION_ID`), ADD UNIQUE KEY `sss` (`REGION_NAME`);
